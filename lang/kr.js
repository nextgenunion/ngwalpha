// 한국어 (kr.js)
// Self-registers into window.SONGBOOK_LANG under the "kr" key.
window.SONGBOOK_LANG = window.SONGBOOK_LANG || {};
window.SONGBOOK_LANG.kr = {
  meta: { name: "한국어" },

  appTitle: "찬양집",
  searchPlaceholder: "제목, 번호 또는 가사로 검색…",
  filterLabelsTitle: "라벨로 필터",
  filterLabelsEmpty: "이 찬양집에는 아직 사용 중인 라벨이 없습니다.",
  filterLabelsClear: "필터 지우기",
  searchOptionsTitle: "검색 옵션",
  sortByAlpha: "가나다순",
  sortByNumber: "번호순",
  sortAsc: "오름차순",
  sortDesc: "내림차순",
  resultsAll: (n) => `${n}곡`,
  resultsFiltered: (n, total) => `${total}곡 중 ${n}곡`,
  emptyState: "찬양집 속에 그런 곡은 없네요. 언젠가 추가될지도 몰라요 😉",
  songLoadError: "곡 목록을 불러오지 못했습니다. 나중에 다시 시도해 주세요.",
  navSongs: "찬양",
  navSettings: "설정",
  navUserSongs: "내 곡",
  backAria: "곡 목록으로 돌아가기",
  keyLabel: "조:",
  altTitlesPrefix: "다른 이름:",
  transposeReset: "초기화",
  lyricsGroup: "가사",
  chordsGroup: "코드",
  chordStyleGroup: "코드 스타일",
  chordStyleSub: "가사 위에 코드를 표시하는 방식",
  chordStyleChip: "칩",
  chordStyleText: "텍스트",
  hideChordsTitle: "코드 숨기기",
  hideChordsSub: "코드 없이 가사만 표시",
  lyricsWeightTitle: "가사 스타일",
  lyricsWeightSub: "가사 텍스트의 글자 굵기",
  lyricsWeightNormal: "보통",
  lyricsWeightSemibold: "세미볼드",
  lyricsWeightBold: "굵게",
  lyricsSpacingTitle: "줄 간격",
  lyricsSpacingSub: "가사 줄 사이의 세로 간격",
  lyricsSpacingTight: "좁게",
  lyricsSpacingNormal: "보통",
  lyricsSpacingLoose: "넓게",
  settingsTitle: "설정",
  sectionAppearance: "화면",
  darkModeTitle: "다크 모드",
  darkModeSub: "밝은 테마와 어두운 테마를 전환합니다",
  accentTitle: "강조 색상",
  accentSub: "앱의 강조 색상을 선택하세요",
  songViewGroup: "목록 보기",
  songViewSub: "찬양집 목록에 곡이 표시되는 방식",
  songViewList: "목록",
  songViewCompact: "컴팩트",
  songViewTiles: "타일",
  listenLink: "듣기",
  songLinkLabel: "링크",
  sectionSongs: "찬양",
  displaySettingsTitle: "표시 설정",
  displaySettingsSub: "목록, 코드, 가사 스타일 및 줄 간격",
  dbTitle: "찬양집",
  dbSub: "사용할 찬양집을 선택하세요",
  dbOptionEnglish: "영어",
  dbPickerTitle: "찬양집",
  dbGroupSda: "SDA",
  dbGroupAll: "전체",
  uiLangTitle: "앱 언어",
  uiLangSub: "메뉴와 버튼에 사용할 언어를 선택하세요",
  sectionApp: "앱",
  sectionAbout: "정보",
  aboutNavSub: "버전, 제작진, 소셜 링크",
  creditsHeading: "제작진",
  installTitle: "Next Gen Worship Alpha 설치",
  installSub: "Next Gen Worship Alpha을 홈 화면에 추가하여 오프라인에서 앱처럼 사용하세요",
  installSubInstalled: "Next Gen Worship Alpha이 이 기기에 설치되었습니다",
  installSubIOS: 'iOS에서: Safari의 공유 버튼을 누른 후 "홈 화면에 추가"를 선택하세요',
  installSubInsecure: "지금은 설치할 수 없습니다. 나중에 다시 시도해 주세요.",
  installBtn: "설치",
  installedBadge: "설치 중…",
  installedBadgeDone: "✓ 설치됨",
  versionTitle: "버전",
  versionSub: (v) => `Next Gen Worship ${v}`,
  appName: "Next Gen Worship",
  toastDbSaved: "찬양집 설정이 저장되었습니다",
  toastIosHint: '공유 버튼을 누른 후 "홈 화면에 추가"를 선택하세요',
  toastPressBackAgain: "종료하려면 뒤로 버튼을 한 번 더 누르세요",
  contactBtn: "피드백 보내기",
  reloadTitle: "곡 데이터베이스 새로고침",
  reloadSub: "오래된 것 같으면 모든 곡을 다시 다운로드합니다",
  reloadBtn: "새로고침",
  reloadBtnBusy: "새로고침 중…",
  reloadAppTitle: "앱 다시 불러오기",
  reloadAppSub: "오래된 것 같으면 앱을 완전히 다시 불러옵니다",
  reloadAppBtn: "앱 다시 불러오기",
  reloadAppBtnBusy: "다시 불러오는 중…",
  toastLibraryReloaded: "곡 데이터베이스를 새로고침했습니다",
  toastLibraryOffline: "오프라인 상태입니다 — 이 기기에 저장된 내용을 표시합니다",
  toastLibraryReloadFailed: "새로고침하지 못했습니다 — 인터넷 연결을 확인한 후 다시 시도하세요",
  toastReloadAppOffline: "오프라인 상태입니다 — 기기에 저장된 버전으로 다시 불러옵니다",
  copyEmailAria: "이메일 주소 복사",
  toastEmailCopied: "이메일 주소가 복사되었습니다",
  toastEmailCopyFailed: "복사하지 못했습니다 — 위 주소를 직접 복사해 주세요",
  scriptureVerse: "누구든지 네 연소함을 업신여기지 못하게 하고 오직 말과 행실과 사랑과 믿음과 정절에 있어서 믿는 자에게 본이 되어",
  scriptureRef: "디모데전서 4:12",
  copyrightTerms: "무단 전재 및 재배포 금지. 사전 서면 허가 없이 본 앱 또는 그 내용의 전부 또는 일부를 복사, 수정, 재배포할 수 없습니다.",

  // ---- 내 곡 + 곡 편집기 (v3) ----
  userSongsTitle: "내 곡",
  userSongsEmptyState: "아직 내가 만든 곡이 없어요. +를 눌러 첫 곡을 써보세요.",
  newSongTitle: "새 곡",
  editSongTitle: "곡 수정",
  editorTitleLabel: "제목",
  editorArtistLabel: "아티스트 / 밴드",
  editorKeyLabel: "조",
  editorKeyPlaceholder: "예: G, Am",
  editorLinkLabel: "링크 (선택)",
  editorLinkPlaceholder: "https://…",
  editorLabelsLabel: "라벨",
  editorLyricsLabel: "가사 및 코드",
  editorLyricsHint: "간단한 서식 예시는 정보 버튼을 눌러 확인하세요.",
  editorPreviewLabel: "미리보기",
  editorPreviewEmpty: "위에 가사를 입력하면 여기에 표시됩니다.",
  editorFormatHelpAria: "곡 서식 도움말",
  editorFormatHelpTitle: "곡 서식 작성 방법",
  editorFormatHelpIntro: "가사는 평소처럼 입력하세요. 아래 표시는 필요할 때만 사용하면 됩니다.",
  editorFormatHelpChordTitle: "코드 넣기",
  editorFormatHelpChordBody: "코드가 바뀌는 단어나 음절 바로 앞에 코드를 [대괄호] 안에 적으세요.",
  editorFormatHelpSectionsTitle: "새 부분 시작하기",
  editorFormatHelpSectionsBody: "절, 후렴, 브리지 같은 부분 사이에는 빈 줄 하나를 남기세요.",
  editorFormatHelpLabelTitle: "부분 이름 붙이기",
  editorFormatHelpLabelBody: "부분의 시작에 이름과 콜론을 적으세요. 예: 후렴: 또는 브리지:. 이름을 쓰지 않으면 앱이 부분에 자동으로 번호를 붙입니다.",
  editorFormatHelpTypeLabel: "이렇게 입력",
  editorFormatHelpPreviewLabel: "이렇게 표시",
  editorFormatHelpExampleSource: "절: [G]놀라운 [C]은혜\n참 아름다운 소리\n\n후렴: [G]나는 길을 잃었지만\n이제 찾았네",
  deleteSongBtn: "곡 삭제",
  deleteSongTitle: "곡 삭제",
  deleteSongConfirm: (title) => `"${title}" 곡을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
  toastSongTitleRequired: "저장하기 전에 곡 제목을 입력해 주세요",
  toastSongCreated: "곡이 생성되었습니다",
  toastSongUpdated: "곡이 수정되었습니다",
  toastSongDeleted: "곡이 삭제되었습니다",
  toastSongSaveFailed: "저장하지 못했습니다 — 다시 시도해 주세요",

  // 라벨 (Labels) — see js/app.js's "Labels" section. 공식/개인 곡 모두에
  // 적용되며, 기기에만 저장됩니다.
  labelsEditTitle: "라벨 편집",
  labelInputPlaceholder: "라벨 추가…",
  labelAddCustomOption: (text) => `"${text}" 추가`,
  labelsNoneYet: "아직 라벨이 없습니다",
  editLabelsBtn: "라벨",
  removeLabelAria: "라벨 삭제",
  labelPresetChristmas: "성탄절",
  labelPresetEaster: "부활절",
  labelPresetCommunion: "성찬식",
  labelPresetBaptism: "세례",
  labelPresetWedding: "결혼식",
  labelPresetFuneral: "장례식",
  labelPresetPraise: "찬양",
  labelPresetWorship: "예배",
  labelPresetKids: "어린이",
  labelPresetChoir: "합창",
  labelPresetOpening: "개회",
  labelPresetClosing: "폐회",

  navPlaylists: "재생목록",
  playlistsTitle: "재생목록",
  favoritesName: "즐겨찾기",
  playlistsEmptyState: "아직 재생목록이 없어요. +를 눌러 첫 목록을 만들어보세요.",
  playlistViewEmptyState: "이 목록이 조금 허전하네요. 좋아하는 곡으로 채워볼까요 🎵",
  playlistViewEmptyStateFavorites: "즐겨찾는 곡이 아직 없나요? :(",
  playlistSongCount: (n) => `${n}곡`,
  newPlaylistTitle: "새 재생목록",
  deletePlaylistTitle: "재생목록 삭제",
  deletePlaylistConfirm: (name) => `"${name}"을(를) 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
  playlistNamePlaceholder: "재생목록 이름",
  cancelBtn: "취소",
  saveBtn: "저장",
  deleteBtn: "삭제",
  addToPlaylistTitle: "재생목록에 추가",
  presentationModeBtn: "프레젠테이션 모드",
  exitPresentationModeBtn: "프레젠테이션 모드 종료",
  addSongsTitle: "곡 추가",
  removeFromPlaylist: "재생목록에서 제거",
  menuDelete: "삭제",
  toastAddedFavorites: "즐겨찾기에 추가되었습니다",
  toastRemovedFavorites: "즐겨찾기에서 제거되었습니다",
  toastPlaylistCreated: "재생목록이 생성되었습니다",
  toastPlaylistDeleted: "재생목록이 삭제되었습니다",
  toastSongRemoved: "재생목록에서 삭제됨",
  undoBtn: "실행 취소",
  playlistsBackupTitle: "재생목록 백업",
  playlistsBackupSub: "재생목록을 파일로 복사하여 다른 사람과 공유하세요.",
  userSongsBackupTitle: "내 곡 백업",
  userSongsBackupSub: "내 곡을 파일로 복사하여 다른 사람과 공유하세요.",
  landscapeModeTitle: "가로 모드",
  landscapeModeSub: "휴대폰을 가로로 돌렸을 때 화면 공간을 더 확보합니다",
  hideVerseNumbersTitle: "절 번호 숨기기",
  hideVerseNumbersSub: "각 절/후렴 위의 작은 \"1, 2, 3…\" 번호를 숨깁니다. \"Bridge:\"같은 명시적 라벨은 계속 표시됩니다.",
  exportBtn: "내보내기",
  importBtn: "가져오기",
  toastPlaylistsExported: "재생목록을 내보냈습니다",
  toastPlaylistsImported: "재생목록을 가져왔습니다",
  toastPlaylistsImportFailed: "파일을 가져오지 못했습니다",
  toastUserSongsExported: "내 곡을 내보냈습니다",
  toastUserSongsImported: "내 곡을 가져왔습니다",
  toastUserSongsImportFailed: "파일을 가져오지 못했습니다",
  editBtn: "편집",
  doneBtn: "완료",
  reorderHandle: "끌어서 순서 변경",
  songOptionsAria: "곡 옵션",

  // Trash bin — Settings → Songs → "Trash bin". See the "Trash Bin page"
  // section in js/app.js.
  trashNavTitle: "휴지통",
  trashNavSub: "삭제된 내 곡은 30일 동안 보관된 후 완전히 삭제됩니다",
  trashTitle: "휴지통",
  trashHint: "삭제된 곡은 이곳에 30일간 보관된 후 완전히 삭제됩니다.",
  trashEmptyState: "휴지통이 비어 있습니다.",
  trashDaysLeft: (n) => `${n}일 남음`,
  trashSelectBtn: "곡 선택",
  trashSelectAllBtn: "전체 선택",
  trashDeselectAllBtn: "선택 해제",
  trashSelectedCount: (n) => `${n}개 선택됨`,
  trashRecoverBtn: "복구",
  deleteTrashConfirmOne: (title) => `"${title}" 곡을 완전히 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
  deleteTrashConfirmMany: (n) => `${n}곡을 완전히 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
  toastTrashRecovered: (n) => n === 1 ? "곡을 복구했습니다" : `${n}곡을 복구했습니다`,
  toastTrashDeleted: (n) => n === 1 ? "곡을 완전히 삭제했습니다" : `${n}곡을 완전히 삭제했습니다`,

  // Easter egg: greeting shown by the small mascot that appears on the
  // Songbook page on Saturdays (Sabbath). See initSabbathMascot() in app.js.
  sabbathGreeting: "행복한 안식일 되세요!",

  // Developer options — its own dedicated page, reached via a hidden
  // Settings row unlocked by tapping the About page's app icon 3 times.
  // See initDevOptions() in app.js.
  toastDevUnlocked: "개발자 옵션이 열렸습니다",
  sectionDevOptions: "개발자 옵션",
  sectionDevEasterEggs: "이스터에그",
  devSabbathTitle: "안식일 마스코트 강제 활성화",
  devSabbathSub: "요일과 상관없이 행복한 안식일 인사를 전하는 예수님 마스코트를 항상 표시합니다.",
  devChristmasTitle: "크리스마스 눈 강제 활성화",
  devChristmasSub: "요일이나 월과 상관없이 크리스마스 눈 모드를 항상 표시합니다.",
  devPartyTitle: "파티 모드 강제 활성화",
  devPartySub: "\"강조 색상\"을 3번 탭하지 않고 미리 볼 수 있습니다. 이 옵션을 끄더라도 비활성화되지 않습니다 — 그 3탭 트리거는 그대로 작동합니다.",
  sectionDevDisplay: "화면",
  sectionDevInProgress: "아직 준비 중",
  devTradMongolianTitle: "전통 몽골 문자",
  devTradMongolianSub: "몽골 문자 옵션을 엽니다.",
  devCreditsTitle: "크레딧 표시",
  devCreditsSub: "정보 페이지에 크레딧 목록을 표시합니다",
  devHideDescTitle: "설정 설명 숨기기",
  devHideDescSub: "일부 설정 항목 아래의 설명 텍스트를 숨깁니다",
  devVividGlassTitle: "곡 상단 유리 효과 선명하게",
  devVividGlassSub: "곡 화면 상단의 프로스트 유리 효과를 기본값인 흰색 계열 대신 더 화려한 색상으로 전환합니다. 라이트 모드에만 적용되며 다크 모드는 영향을 받지 않습니다.",
};
